...
function concatenateStrings(value1, value2) {
  return value1.concat(value2).trim();
}
...
function extractNameFromTemplate(value) {
  return value.slice(7).slice(-value.length, -1);
}
...
function extractEmails(str) {
  const arr = str.match(/\w+.\w+@\w+\.\w+(;|$)/g).map((item) => item.replace(/;/gm, ''));
  return arr;
}
...
function getRectangleString(width, height) {
  ...
  if (width && height !== 2) {
    arr.fill(`│${' '.repeat(width - 2)}│\n`);
  }
  arr[0] = `┌${'─'.repeat(width - 2)}┐\n`;
  arr[arr.length - 1] = `└${'─'.repeat(width - 2)}┘\n`;
  return arr.join('');
}
...
function encodeToRot13(str) {
  ...
  str.split('').forEach((item) => {
    if (item === ' ') {
      ...
      result.push(rot[alphabet.indexOf(item)]);
    } else {
      result.push(item);
    }
  });
